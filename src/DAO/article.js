...
const getList = async (start, count) => {
  const sql =
    "SELECT a.id, a.title, a.created_at AS createdAt, a.last_updated AS lastUpdated, " +
    "u.display_name AS displayName from articles AS a INNER JOIN users AS u " +
    "ON u.id = a.author WHERE a.is_active = 1 AND a.is_deleted = 0 " +
...
const getTotalCount = async () => {
  const sql =
    "SELECT count(*) AS totalCount FROM articles WHERE is_active = 1 AND is_deleted = 0";
  const { totalCount } = runQuery(sql, []);
  return totalCount;
};

const getById = async (id) => {
  const sql =
    "SELECT a.id, a.title, a.content, a.created_at AS createdAt, a.last_updated AS lastUpdated, a.author, " +
    "u.display_name AS displayName from articles AS a INNER JOIN users AS u " +
    "ON u.id = a.author WHERE a.id = ? AND a.is_active = 1 AND a.is_deleted = 0";
...
const getByIdAndAuthor = async (id, author) => {
  const sql =
    "SELECT title, content, author, created_at AS createdAt, last_updated AS lastUpdated FROM articles " +
